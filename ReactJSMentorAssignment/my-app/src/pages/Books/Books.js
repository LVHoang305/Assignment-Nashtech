...
import { apiGetBooks, apiDeleteBooks } from "../../services/Books"; // Make sure these services are correctly implemented
      ...
      {error ? (
        ...
        <table className="w-full bg-white table-fixed">
          ...
          <tbody>
            {!filteredBooks ? (
              <tr>
                <td colSpan="6" className="text-center">There is no book!</td>
              </tr>
            ) : (
                ...
                return (
                  <tr className="h-[50px]" key={item.id}>
                    ...
                    <td className="border text-center p-2">
                      <img src={item?.imageLink} alt={item?.title} className="h-16 mx-auto" />
                    </td>
                    <td className="border text-center p-2 truncate">{item?.title}</td>
                    <td className="border text-center p-2 truncate">{item?.author}</td>
                    <td className="border text-center p-2 truncate">{item?.country}</td>
                    <td className="border text-center p-2 truncate">{item?.language}</td>
                    <td className="border text-center p-2">{item?.year}</td>
                    <td className="border text-center p-2">{item?.pages}</td>
